...
const FacturasCompraModule = {
    ...
    renderPreFacturas: (container) => {
        ...
        container.querySelectorAll('.btn-print-pre').forEach(btn => {
            btn.addEventListener('click', () => {
                FacturasCompraModule.printPreInvoice(btn.dataset.id);
            });
        });
    },

    printPreInvoice: async (id) => {
        const facturas = window.erpDB.getAll('facturas_compra');
        const fac = facturas.find(f => f.id === id);
        if (!fac) return alert("Pre-factura no encontrada");

        try {
            const response = await fetch('views/pre_factura_plantilla.html');
            ...
            const replacements = {
                '{{FOLIO}}': fac.id,
                '{{OC_ID}}': fac.oc_id,
                '{{PROVEEDOR_NOMBRE}}': fac.proveedor,
                '{{BANCO}}': fac.banco,
                '{{CLABE}}': fac.clabe,
                '{{FECHA}}': fac.fecha,
